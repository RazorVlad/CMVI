...
public class Krylov {
	...
	private void makeBsystem() {
		...
		for (int i = 1; i < n; i++) {
			powA[i] = GaussSeidel.matrixMultiplication(getA(), powA[i - 1]);
			for (int t = 0; t < n; t++)
				for (int k = 0; k < n; k++)
		...
		for (int i = 0; i < n - 1; i++) {
			transLeft[i] = GaussSeidel.vectMatrMultiplication(powA[n - i - 2],
					x0);
		}
		for (int i = 0; i < n; i++)
			transLeft[n - 1][i] = x0[i];
		setLeftB(GaussSeidel.transMatrix(transLeft));
		setRightB(GaussSeidel.vectMatrMultiplication(powA[n - 1], x0));
...
//			System.out.println("number " + i + " " + numbers[i]);
	...
	public void findVectors() {
		...
		if (numbers.length == 0) {
			...
			double[][] x = GaussSeidel.transMatrix(leftB);
