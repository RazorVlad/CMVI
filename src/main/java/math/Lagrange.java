...
public class Lagrange {
	...
	public void setX(double[] x) {
		this.x = new ArrayList<Double>();
		for (double aX : x) this.x.add(aX);
	}

	public void setY(double[] y) {
		this.y = new ArrayList<Double>();
		for (double aY : y) this.y.add(aY);
	}
	...
	public ArrayList<Double> PolSum (ArrayList<Double> x, ArrayList<Double> y) {
	      ...
	      if (x.size() > y.size()) {
	        small_size = y.size();
			  for (Double aX : x) result.add(aX);
	      }
	      else {
	        small_size = x.size();
			  for (Double aY : y) result.add(aY);
	      }
	     ...
	     //   out.println("I: " + i + "J: " + j + "<br />");
	        ply = true;
	          if (i != j) {
	            ...
	            if (ply == true) {
	            previous.clear();
					for (Double anUp : up) previous.add(anUp);
	            }
...
//	                out.println("ERROR");
	    ...
	    //  out.println("DEL: " + PolToString(pol));
	      ...
	      tmp2.clear();
			for (Double aPol : pol) tmp2.add(aPol);

	      if (i==0) {
			  for (Double aTmp2 : tmp2) sum.add(aTmp2);
	      }
	      else {
	        tmp.clear();
	      ...
	      //  out.println("<br/>DVA: " + sum);
	        ...
	        sum.clear();
			  for (Double aTmp : tmp) sum.add(aTmp);
	      }
